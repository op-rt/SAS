...
For lack of a name this algorithm will be referred to as "Stream And Scan" (SAS). Unlike other datastructures such as RTrees, Quad-trees or kd-Trees, this Segment Tree is **streamed** on the fly, without explicitly being stored, before **scanning**. The name is also a direct reference to SAP (Sweep and Prune).
